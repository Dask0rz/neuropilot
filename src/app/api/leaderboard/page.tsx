    ...
    <div className="min-h-screen p-6 max-w-2xl mx-auto">
      ...
      {!quizExists ? (
        ...
        <div className="text-center py-20">
          ...
          <button
            onClick={() => router.push('/dashboard/weekly-quiz')}
            className="mt-4 px-6 py-3 bg-cyan-500 text-black font-bold rounded-xl"
          >
            Jouer maintenant
          </button>
        </div>
        ...
        <>
          ...
          <div className="space-y-2">
            {leaderboard.map((entry) => (
              ...
              >
                <span className={`font-black text-lg w-8 text-center ${getRankStyle(entry.rank)}`}>
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-2">
                    ...
                    {entry.isCurrentUser && (
                      <span className="text-xs bg-cyan-400/20 text-cyan-400 px-2 py-0.5 rounded-full">Toi</span>
                    )}
                  </div>
          ...
          {/* Bouton jouer si pas encore participé */}
          {!leaderboard.some(e => e.isCurrentUser) && (
            <button
              onClick={() => router.push('/dashboard/weekly-quiz')}
              className="mt-6 w-full py-4 bg-gradient-to-r from-cyan-500 to-purple-500 text-black font-black rounded-xl text-lg"
            >
              🎮 Participer au quiz !
            </button>
          )}
